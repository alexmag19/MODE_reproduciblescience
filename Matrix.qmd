...
Il n’y qu’une petite partie de ces juvéniles qui parvient à atteindre la classe d’âge adulte, en suivant un taux de passage noté TPa.
...
-	40 % des femelles suricates seniors qui sont encore vivantes à la fin de leur onzième année vont survivre, mais pas au-delà d’une douzième année.
...
Cette matrice est construite de telle sorte à ce que nous ayons en colonne dans l'ordre Jt+1, At+1, St+1 et en ligne Jt, At et St. 
...
Sur la diagonale nous aurons les paramètres concernant la survie et sur la diagonale en dessous ceux concernant les probabilité de passer
d'un stade d'âge, à un autre. 

Ainsi, si nous remplaçons par nos valeurs nous obtenons la matrice suivante :  
...
En l'occurence, dans notre exemple nous l'obtenons ainsi : 

```{r}
taux_de_croissance = lambda(matrice)
```

Nous obtenons alors un taux de croissance de 1.795362. 
...
ou une combinaison de ces éléments. Ainsi, cela indique que notre population est dynamique et qu'elle est en croissance démographique. 
Si le taux de croissance avait été inférieur à 1, cela aurait indiqué que notre population serait en déclin. 
Généralement, ces types de taux de croissance sont associés à des populations vieillissantes, aux tendances démographiques négatives.
Ceci n'est pas le cas dans notre population de suricates. 
...
Le temps de génération représente la durée moyenne entre la naissance d'une génération parentale et la naissance de la génération suivante, ce qui nous permet ainsi d'avoir une perspective sur la manière dont les caractéristiques démographiques se transmettent à travers les générations.

On va désormais calculer la structure d'âge stable, qui sera sous forme de proportion par classe d'âge et qui permet de représenter la répartition attendue des différents 
groupes d'âge dans une population stationnaire. Pour obtenir ces proportions, on suppose généralement que la population est stable dans le sens où le nombre de naissances, le nombre de décès et le nombre de migrants sont équilibrés et que la distribution par âge reste constante. 
...
On peut aussi calculer les valeurs reproductives à chaque classe d'âge, ce qui va nous permettre d'évaluer la contribution de chaque groupe d'âge à la reproduction et à la croissance de la population. La valeur reproductive d'une classe d'âge est calculée en tenant compte des taux de survie et des taux de reproduction spécifiques à chaque classe d'âge. Elle représente le nombre moyen de descendants qu'un individu d'une classe d'âge particulière peut s'attendre à produire au cours de sa vie.
...
Nous obtenons ainsi : 1 correspondant à l'indice de base (classe juvénile), 3.59 (classe adulte), 0.71 (classe sénior). Etant donné que les juvéniles ne peuvent pas se reproduire, on ne prend pas en compte cette classe là, d'où le 1. 
Nous voyons bien que ce sont les adultes qui contribuent le plus à la reproduction et que les séniors contribuent un tout petit peu. 

Nous pouvons ensuite calculer le temps qu'il faudra à notre population pour se remettre d'une perturbation.
...
Nous voyons ainsi qu'il faudrait environ 11 ans à notre population pour se remettre d'une perturbation comme une catastrophe naturelle, un changement environnemental etc...
...
Enfin, à l'aide de la fonction elasticity(), on va pouvoir voir que le taux le plus grand va le plus contribuer à l'évolution de notre population : 
